...
export default function ClipboardManager() {
  ...
  const itemsCollectionRef = collection(db, 'clipboardItems');
   ...
   const handlePasteFromClipboard = async () => {
    try {
      ...
      if (clipboardContent) {
        ...
        toast({ title: 'Empty Clipboard', description: 'Nothing found in clipboard to paste.', variant: 'destructive' });
      }
    } catch (error) {
      console.error('Failed to read clipboard contents: ', error);
      toast({ title: 'Error', description: 'Could not access clipboard. Check permissions.', variant: 'destructive' });
    }
  };
  ...
  const renderItemContent = (item: ClipboardItem) => {
    switch (item.type) {
      ...
      case 'image':
        return (
            <div className="relative w-full max-h-60 overflow-hidden rounded-md my-2">
              <Image
                src={item.content}
                alt={item.fileName || 'Uploaded image'}
                width={300} // Adjust width as needed
                height={200} // Adjust height as needed
                style={{ objectFit: 'contain' }} // Use 'contain' or 'cover'
      ...
      case 'file':
        return (
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="truncate">{item.fileName || 'Uploaded file'}</span>
          </div>
        );
      default:
    ...
    <div className="w-full max-w-2xl mx-auto space-y-8">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              className="file:text-accent file:border-accent hover:file:bg-accent/10"
              disabled={isUploading}
            />
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <p className="text-muted-foreground text-center py-4">Your shared clipboard is empty.</p>
        ) : (
